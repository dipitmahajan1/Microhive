...
5. What I did to meet each evaluation criteria :
   ...
   **Disclaimer** :  THE SCRIPT TO 100 % EXTRACT PDF FROM EVERY PDF IS NOT POSSIBLE WITHOUT USING IN HOUSE ML MODEL OR WITHOUT FINE TUNING NER MODEL. IN THE GIVEN TIME CONSTRUCTING AN ETL SCRIPT 
   WITH BEST ACCURACY WAS THE PRIORITY.THIS SCRIPT CAN BE USED TO EXTRACT ALL PDF PAPERS BUT HAS BEEN WRITTEN TO GIVE HIGHEST ACCURACY WITH PAPER1 BECAUSE IT TAKES FEW ASSUMPTION WITH RESPECT TO
   WHERE AUTHOR,TITLE WOULD RESIDE IN THE PDF. THUS ALL IMAGES AND JSON FORMED SHOW PAPER1 AND IT IS THE ONE BEING STORED IN DYNAMO DB. WITH FEW CHANGES WE CAN STORE SUBSEQUENT PAPERS TOO.
